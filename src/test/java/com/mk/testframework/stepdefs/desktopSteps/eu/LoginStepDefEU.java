...
public class LoginStepDefEU {
	...
	@Inject
	public LoginStepDefEU(MainMenuStepDef mainMenuStepDef, MkCreateAccountPage_Common mkCreateAccountPageCommon,
			TestData testData, MkHamburgerMenu mkHamburgerMenu, MkPageNavigator mkPageNavigator,
			MkCreateAccountPage_EU mkCreateAccountPageEU, Context context, MkMyAccountPage_EuGlobal mkMyAccountPage_EuGlobal) {
	...
	@And("Login: I enter US user email address")
	public void i_enter_us_username() {
		mkCreateAccountPageCommon.enterEmailAddress(testData.getUser().getEmail());
	...
	public void i_should_be_successfully_logged_out() {
		mkPageNavigator.goToHomePage();
		Assert.assertTrue("Expecting sign in link to ensure user is logged out", mkHamburgerMenu.signInLinkIsPresent());
	}

	@And("Login: I enter new created email address")
	public void i_enter_new_created_user() {
		mkCreateAccountPageEU.enterCreatedEmailAddress(context.getSavedData("email"));

	}

	@And("Login: I enter changed password")
	public void i_enter_changed_password() {
		mkCreateAccountPageEU.enterPassword(testData.getUser().getPassword() + "234");
	}
	...
	Assert.assertTrue(mkCreateAccountPageEU.isLoginErrorDisplaying());
	}
